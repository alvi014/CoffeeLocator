...
public class CoffeeShopsController : ControllerBase
{
    ...
    /// <summary>
    /// Retrieves all registered coffee shops mapped to DTOs.
    /// </summary>
    /// <returns>A list of coffee shop response objects.</returns>
    [HttpGet]
    ...
    [HttpGet("{id}")]
    public async Task<ActionResult<CoffeeShopResponseDto>> GetCoffeeShop(Guid id)
    {
    ...
    {
        var nearbyShops = await _coffeeShopService.GetNearbyShopsAsync(userLat, userLon);
    ...
    [HttpPost]
    public async Task<ActionResult<CoffeeShopResponseDto>> PostCoffeeShop(CreateCoffeeShopDto dto)
    {
